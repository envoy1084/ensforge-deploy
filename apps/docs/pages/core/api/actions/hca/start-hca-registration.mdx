...
```ts
import { startHcaRegistration } from "@ensforge/core/hca";
```
...
type Input = Parameters<typeof startHcaRegistration>[1];
declare const parameters: Input;

const result = await startHcaRegistration(config, parameters);
```
...
```ts
type Result = Awaited<ReturnType<typeof startHcaRegistration>>;
```
...
const program = startHcaRegistration.effect(config, parameters);
...
- [`startHcaRegistration`](/sdk/api/hca/start-hca-registration)
