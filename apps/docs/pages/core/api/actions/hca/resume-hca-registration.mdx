...
type Input = Parameters<typeof resumeHcaRegistration>[1];
declare const parameters: Input;

const result = await resumeHcaRegistration(config, parameters);
```
...
```ts
type Result = Awaited<ReturnType<typeof resumeHcaRegistration>>;
```
...
const program = resumeHcaRegistration.effect(config, parameters);
