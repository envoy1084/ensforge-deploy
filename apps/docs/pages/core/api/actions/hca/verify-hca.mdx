...
const result = await verifyHca(config, {
...
```ts
import type { VerifyHcaParameters } from "@ensforge/core/hca";
```
...
```ts
import type { VerifiedHcaAccount } from "@ensforge/core/hca";
```
...
const program = verifyHca.effect(config, {
  hca: "0x1234567890123456789012345678901234567890",
});
...
```ts
const request = verifyHca.request({
  hca: "0x1234567890123456789012345678901234567890",
});
```
