---
title: prepareHcaCalls
description: Validate raw calls or ENS intents into an HCA plan.
---

import EffectGuide from "../../../../../shared/core/effect.mdx";
import ErrorGuide from "../../../../../shared/core/error.mdx";

# prepareHcaCalls

Validate raw calls or ENS intents into an HCA plan.

## Import

```ts
import { prepareHcaCalls } from "@ensforge/core/hca";
```

## Usage

:::code-group

```ts [index.ts]
import { prepareHcaCalls } from "@ensforge/core/hca";
import { config } from "./config";

type Input = Parameters<typeof prepareHcaCalls>[1];
declare const parameters: Input;

const result = await prepareHcaCalls(config, parameters);
```

```ts [config.ts]
// [!include ~/snippets/hca/config.ts]
```

:::

## Parameters

```ts
type Input = Parameters<typeof prepareHcaCalls>[1];
```

### counterfactualOwner

`` `0x${string}` | undefined ``

Expected owner for counterfactual execution.

### operationId

`string | undefined`

Optional caller correlation identifier for execution tracking.

### requiredCapabilities

`readonly ("ownerExecution" | "sessionExecution" | "counterfactualDeployment" | "atomicBatching" | "sponsorship" | "crossChainFunding")[] | undefined`

Capabilities that the execution route must support.

### hca

`` `0x${string}` ``

HCA address to inspect or operate on.

### salt

`bigint | undefined`

Account derivation salt. Defaults to the deployment profile canonical salt.

### authorization

`HcaAuthorization`

Owner authorization or an enabled destination session.

### calls

``readonly ({ readonly to: `0x${string}`; readonly data?: `0x${string}` | undefined; readonly value?: bigint | undefined; } | EnsWriteIntent<unknown, WriteError>)[]``

Ordered raw contract calls or supported ENS .call intents.

### walletClient

`Input["walletClient"]`

Viem wallet override. Defaults to the configured wallet resolver.

### account

`` `0x${string}` | Account | undefined ``

Wallet account override for the operation.

## Return Type

```ts
type Result = Awaited<ReturnType<typeof prepareHcaCalls>>;
```

Returns `PreparedHcaCalls`.

## Effect

<EffectGuide />

```ts
import { Effect } from "effect";

const program = prepareHcaCalls.effect(config, parameters);
type Success = Effect.Success<typeof program>;
type Failure = Effect.Error<typeof program>;
const result = await Effect.runPromise(program);
```

## Error

<ErrorGuide />

Deployment, account and authorization mismatches remain explicit errors. Reconcile uncertain submissions before retrying a write.

## Related

- [`prepareHcaCalls`](/sdk/api/hca/prepare-hca-calls)
- [Guide](/core/guides/hca)
