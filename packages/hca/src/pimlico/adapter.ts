...
import { hashOperation, maximumGasCost, pimlicoCall } from "./operation.js";
      ...
      pimlicoCall(async () => {
        ...
        const client = createSmartAccountClient({
          ...
          ...(options.sponsorship?.policyId
            ...
            : {}),
          userOperation: {
            estimateFeesPerGas: async () => (await options.client.getUserOperationGasPrice()).fast,
          },
        });
        ...
        const maximum = options.sponsorship ? 0n : maximumGasCost(operation);
        return {
    ...
    getStatus: defineAction((config, submission) =>
      pimlicoCall(async () => {
        ...
        if (
          ...
          !isAddressEqual(submission.locator.entryPoint, options.profile.infrastructure.entryPoint)
        )
          throw new HcaError({
            code: "INVALID_SUBMISSION",
        ...
        const event = events.find(
          (log) => log.args.userOpHash === hash && isAddressEqual(log.args.sender, submission.hca),
        );
