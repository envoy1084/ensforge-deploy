...
) => Effect.Effect<HcaWiringVerification, TestEnvironmentError> = Effect.fn("verifyHcaDeployment")(
  function* (client: PublicClient, profile: HcaDeploymentProfile) {
    return yield* Effect.tryPromise({
      try: async () => {
        const { contracts: hca, deployment, infrastructure } = profile;
        if ((await client.getChainId()) !== deployment.chainId)
          throw new Error("HCA chain mismatch");
        if (
          !(
